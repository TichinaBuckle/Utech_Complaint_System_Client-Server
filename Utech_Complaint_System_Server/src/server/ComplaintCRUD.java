...
public class ComplaintCRUD {
 ...
 // Retrieve a single Complaint by its id
    ...
    // Get Advisors Full name using their id
    public String getAdvisorFullName(Complaint complaint) throws SQLException {
        String advisorFullName = null;
        String query = "SELECT CONCAT(first_name, ' ', last_name) AS full_name FROM account WHERE id = ?";
        ...
        try (Connection connection = dataSource.getConnection();
            ...
            if (resultSet.next()) {
                advisorFullName = resultSet.getString("full_name");
            }
        }
